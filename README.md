...
3. 在 `backend/.env` 中填写一枚新建的 `GROQ_API_KEY` 和随机生成的 `JWT_SECRET`；`GROQ_MODEL` 默认使用 `openai/gpt-oss-20b`。
...
- Render 后端配置 `GROQ_API_KEY`、`GROQ_MODEL`、`JWT_SECRET` 和 `PORT`；不要把真实密钥提交到 Git。
